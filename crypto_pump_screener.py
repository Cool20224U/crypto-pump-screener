...
import ccxtimport streamlit as st
import pandas as pd
import pandas_ta as ta
import ccxt
import requests
...
# FIX 1: Initialize signal_history before reading from file
...
def get_social_spike(symbol, lunar_key):
    # FIX 3: Accept lunar_key as a parameter instead of relying on outer scope
    if not lunar_key:
        return "No key"
    try:
        r = requests.get(
...
def calculate_pump_score(rvol, ohlcv_df, min_rvol):
    # FIX 4: Use a clearly named parameter to avoid shadowing
    score = 0
    if rvol >= min_rvol:
        score += 35
...
def scan_coins(min_rvol, max_24h_gain, lunar_key):
    # FIX: Accept settings as parameters
    coins = get_top_300()
    if coins.empty:
    ...
    for _, coin in coins.iterrows():
        ...
        try:
            ...
            # FIX 4: Use a distinct variable name (ohlcv_df) to avoid shadowing outer df
            ...
            if (ema_cross or rvol >= min_rvol) and macd_bull:
                ...
                signal = {
                    ...
                    "RVOL": round(rvol, 1),
                    "1h %": round(coin.get('price_change_percentage_1h_in_currency', 0) or 0, 2),
...
with tab1:
    ...
    st.caption(
        f"🕒 Last scan: {dubai_tz.strftime('%Y-%m-%d %H:%M:%S')} **Dubai time** | "
    ...
    if not df_signals.empty:
        live_display = df_signals.head(5)import streamlit as st
import pandas as pd
import pandas_ta as ta
import ccxt
import requests
...
with tab1:
    ...
    if not coins.empty:
        ...
        # FIX 2: Use correct column name for 1h % from CoinGecko response
        one_h_col = (
            'price_change_percentage_1h_in_currency'
            if 'price_change_percentage_1h_in_currency' in hot.columns
            else 'price_change_percentage_1h'
        )

        display_hot = pd.DataFrame({
            "Coin": hot["symbol"].str.upper().values,
            "Price ($)": hot["current_price"].round(6).values,
            "1h %": hot[one_h_col].fillna(0).round(2).values,
